...
def repo_file(repo, *path, mkdir=False):
  ...
  repo_file(r, \"refs\", \"remotes\", \"origin\", \"HEAD\") will create .git/ref/remotes/origin."""

  if repo_dir(repo, *path[:-1], mkdir=mkdir):
    return repo_path(repo, *path)

def repo_dir(repo, *path, mkdir=False):
  """Same as repo_path, but mkdir *path if absend if mkdir."""
...
def repo_create(path):
  ...
  # First, we make sure tha path either doesn't exist or is an empty dir.
